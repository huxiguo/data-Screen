<template>
  ...
  <div class="actual-total">
    <div class="expect-total">可预购总量<i>999999</i>件</div>
    <div class="actual-total">
      <div class="actual-item" v-for="(item, index) in actualTotal.toString().padStart(7, '0').split('')" :key="index">
        {{ item }}
      </div>
    </div>
  </div>
...
const actualTotal: Ref<any> = ref(0)
function generateRandomNum(): number {
  return Math.floor(Math.random() * 100) + 1
}

setInterval(() => {
  actualTotal.value += generateRandomNum()
  if (actualTotal.value > 9999999) {
    actualTotal.value = 0
  }
}, 1000)
    ...
    series: [
      {
        ...
        z: 2,
        center: ['50%', '50%'],
        data: [data, data, data, data], // data个数代表波浪数
        itemStyle: {
          color: {
            type: 'linear',
            x: 0,
